import { v4 as uuidv4 } from 'uuid';

const initialSection = {
  name: 'Breakfast',

  id: uuidv4(),
  items: [
    {
      name: 'The southern Breakfast',
      price: '$ 9',
      descreption:
        '2 egg any style with choice of meal (ham, bacon or sausage)',
    },
    {
      name: 'Cajun Sausuge Omelet',
      price: '$ 12',
      descreption:
        '3 eggs omelet with beef sausage, parsley, onion, tomato, black cracked pepper and cheddar cheese',
    },
    {
      name: 'Seafood Omelet',
      price: '$ 13',
      descreption:
        '3 eggs omelet with shrimp, crawrish, red pepper and cheddar cheese',
    },
    {
      name: 'Blueberry Pancakes',
      price: '$ 11',
      descreption: '',
    },
    {
      name: 'Brioche French Toast',
      price: '$ 11',
      descreption: '',
    },
    {
      name: 'Crispy Country Omelet',
      price: '$ 12',
      descreption:
        '3 eggs omelet filled with crispy hash brown, ham, cheddar cheese, onion and peppers topped with gravy',
    },
    {
      name: 'Egg White Omelet',
      price: '$ 12',
      descreption: '3 egg white omelet with mushroom, spinach and feta cheese',
    },
    {
      name: 'Veggle Omelet',
      price: '$ 10',
      descreption:
        '3 eggs omelet with mushroom, onions, tomatoes, bell pepper, spinach & cheddar cheese',
    },
    {
      name: 'Cheese Omelet',
      price: '$ 8',
      descreption: '3 eggs omelet',
    },
    {
      name: 'Traditional Buttermilk Pancakes',
      price: '$ 9',
      descreption: 'American classics...',
    },
    {
      name: 'Chocolate Chip Pancakes',
      price: '$ 11',
      descreption: 'Buttermilk pancakes filled with chocolate chips',
    },
    {
      name: 'Buttermilk Waffles',
      price: '$ 9',
      descreption: '',
    },
    {
      name: 'Biscuit & Gravy',
      price: '$ 9',
      descreption: 'Buttermilk biscuit topped with country gravy and sausage',
    },
    {
      name: 'Traditional Benedict',
      price: '$ 12',
      descreption:
        'Ham served over buttermilk biscuits, topped with two poached eggs finished with Hollandaise',
    },
    {
      name: 'Crab Cake Benedict',
      price: '$ 14',
      descreption:
        'Grilled crab cake served over buttermilk biscuit, topped with two poached eggs fınished with hollandaise',
    },
    {
      name: 'Shrimp & Grits',
      price: '$ 12',
      descreption: 'BBQ shrimp over housemade grits',
    },
    {
      name: 'Street Car Energy Booster',
      price: '$ 11',
      descreption:
        'Start your day with our combination of Greek yogurt, honey granola and fresh fruit',
    },
    {
      name: '2 eggs any style',
      price: '$ 3',
      descreption: '',
    },
    {
      name: 'Applewood Bacon',
      price: '$ 2.50',
      descreption: '',
    },
    {
      name: 'Turkey Bacon',
      price: '$ 3.50',
      descreption: '',
    },
    {
      name: 'Ham',
      price: '$ 3',
      descreption: '',
    },
    {
      name: 'Beef Sausage',
      price: '$ 4',
      descreption: '',
    },
    {
      name: 'Toast',
      price: '$ 2',
      descreption: '',
    },
    {
      name: 'Buttermilk Biscuit',
      price: '$ 2',
      descreption: '',
    },
    {
      name: 'Hash Brown',
      price: '$ 3',
      descreption: '',
    },
    {
      name: 'Grits',
      price: '$ 3',
      descreption: '',
    },
  ],
};

const menuSections = [
  initialSection,

  {
    name: 'beignet & coffee',

    id: uuidv4(),
    items: [
      {
        name: 'Beignet',
        price: '$10',
        descreption: '3 egg any style with choice (ham, bacon or sausage)',
      },
      {
        name: 'Latte (Iced or Hot)',
        price: '$ 5',
        descreption: '',
      },
      {
        name: 'Espresso',
        price: '$ 4',
        descreption: '',
      },
      {
        name: 'Americano',
        price: '$ 4',
        descreption: '',
      },
      {
        name: 'Fresh Roasted Coffee',
        price: '$ 3',
        descreption: '',
      },
    ],
  },
  {
    name: 'appetizers',

    id: uuidv4(),
    items: [
      {
        name: 'Cup of Gumbo',
        price: '$ 6',
        descreption: 'Served NOLA style with potato salad',
      },
      {
        name: 'Cup of Red Beans & Rice',
        price: '$ 7',
        descreption: 'Made with beef sausage',
      },
      {
        name: 'Blackened Alligator Bites',
        price: '$ 10',
        descreption: 'Served with remoulade dipping sauce',
      },
      {
        name: 'Crab Cake',
        price: '$ 10',
        descreption: 'Topped with remoulade sauce',
      },
    ],
  },
  {
    name: 'lunch & diner',

    id: uuidv4(),
    items: [
      {
        name: 'Fried Shrimp Po-boy',
        price: '$ 14',
        descreption: '',
      },
      {
        name: 'Crab Cake Burger & Fries',
        price: '$ 14',
        descreption:
          'Fully dressed with mayo, lettuce, tomato, onion and pickles. Add cheese: $1.00 Add bacon: $2.50',
      },
      {
        name: 'Cajun Penne Pasta',
        price: '$ 15',
        descreption:
          'Creamy penne pasta made with beef sausage & diced chicken served with bread',
      },
      {
        name: 'Veggie Pasta',
        price: '$ 15',
        descreption:
          'Creamy penne pasta made with cherry tomatoes, red onion, bell pepper & mushrooms. Served with bread',
      },
      {
        name: 'Fried Seafood Platter',
        price: '$ 20',
        descreption: 'Seasoned golden fried oysters, shrimp and crispy fish',
      },
      {
        name: 'Grilled Cajun Chicken Sandwich',
        price: '$ 12',
        descreption: '',
      },
    ],
  },
  {
    name: 'new orleans',

    id: uuidv4(),
    items: [
      {
        name: 'Bowl of Gumbo',
        price: '$ 12',
        descreption: 'Served with biscuit. NOLA style potato salad',
      },
      {
        name: 'Jambalaya',
        price: '$ 15',
        descreption:
          'Made with chicken breast and beef sausage served with a biscuit',
      },
      {
        name: 'Taste of New Orleans',
        price: '$ 17',
        descreption:
          'Gumbo, jambalaya, red beans and rice with beef sausage served with biscuit',
      },
      {
        name: 'Blackened Redfish',
        price: '$ 19',
        descreption:
          'Gulf redfish blackened with our spacial seasonngs, topped with a creamy mushroom sauce. Served with seasonal veggies and potato salad',
      },
      {
        name: '12 oz. Ribeye',
        price: '$ 23',
        descreption: 'Served with seasonal veggies and potato salad',
      },
      {
        name: 'Grilled Salmon',
        price: '$ 19',
        descreption:
          'Grilled salmon topped with a creamy mushroom sause, served with seasonal veggies and potato salad',
      },
    ],
  },
  {
    name: 'drink specials',
    id: uuidv4(),
    items: [
      {
        name: 'Espresso',
        price: '$ 4',
        descreption: '',
      },
      {
        name: 'Fresh Roasted Coffee',
        price: '$ 3',
        descreption: '',
      },
      {
        name: 'Bloody Mary',
        price: '$ 7',
        descreption: '',
      },
      {
        name: 'Mimosa',
        price: '$ 8',
        descreption: '',
      },
      {
        name: 'Hurricane',
        price: '$ 8',
        descreption: '',
      },
    ],
  },
];

export { menuSections, initialSection };
